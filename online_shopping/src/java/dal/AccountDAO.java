...
//        }
    ...
    public boolean updatePasswordShop(String UserName, String newPassword) {
    boolean isUpdated = false;
    // Câu lệnh SQL để cập nhật mật khẩu dựa trên ShopName
    String query = "UPDATE a " +
                   "SET a.Password = ? " +
                   "FROM [dbo].[Account] a " +
                   "INNER JOIN [dbo].[Shops] s ON a.UserName = s.UserName " +
                   "WHERE s.UserName = ?";

    try (PreparedStatement stmt = connection.prepareStatement(query)) {
        // Thêm tham số vào câu lệnh SQL
        stmt.setString(1, newPassword);  // Cập nhật mật khẩu
        ...
        if (rowsAffected > 0) {
            isUpdated = true; // Nếu ít nhất một dòng được cập nhật
        }
    } catch (SQLException e) {
